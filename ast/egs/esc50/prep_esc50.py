...
def main(args):
    ...
    if args.fold_wise_eval:
        ...
        validation = pd.read_csv(args.valid_csv)
        test = pd.read_csv(args.test_csv)

        for i,row in train.iterrows():
            cur_label = label_map[row['label']]
            cur_dict = {"wav": row['path'], "labels": '/m/07rwj'+cur_label.zfill(2)}
            train_wav_list.append(cur_dict)
        ...
        with open('/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_' + args.dataset_name + '/' + args.dataset_name + '_train_data' +'.json', 'w') as f:
            json.dump({'data': train_wav_list}, f, indent=1)

        with open('/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_' + args.dataset_name + '/' + args.dataset_name + '_val_data' +'.json', 'w') as f:
            json.dump({'data': eval_wav_list}, f, indent=1)

        with open('/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_' + args.dataset_name + '/' + args.dataset_name + '_test_data' +'.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--valid_csv', type=str, help='Path to validation CSV', required=False)
    parser.add_argument('--test_csv', type=str, help='Path to test CSV', required=False)
