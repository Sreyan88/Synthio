...
python ./prep_esc50_supcon.py --label_map_json $label_map --train_csv $supcon_csv --fold_wise_eval $fold_wise_eval --dataset_name $dataset
...
if [ "$fold_wise_eval" == "True" ]; then
  for ((fold=1; fold<=5; fold++)); do
    ...
    CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run.py --model ${model} --dataset ${dataset} \
      ...
      --lr $lr --n-epochs ${epoch} --batch-size 12 --save_model False \
  ...
  tr_data=/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_${dataset}/${dataset}_train_data_supcon.json
  te_data=/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_${dataset}/${dataset}_train_data_supcon.json
  tv_data=/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_${dataset}/${dataset}_train_data_supcon.json

  csv_file="./data/${dataset}_class_labels_indices.csv"
  line_count=$(tail -n +2 "$csv_file" | wc -l)

  python ./gen_weight_file.py --data_path $tr_data --label_csv $csv_file

  CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run_multi_conpos.py --model ${model} --dataset ${dataset} \
    ...
    --lr $lr --n-epochs 100 --batch-size 24 --save_model False \
    --freqm $freqm --timem $timem --mixup ${mixup} --bal "bal" \
    --tstride $tstride --fstride $fstride --imagenet_pretrain $imagenetpretrain --audioset_pretrain $audiosetpretrain \
    --metrics ${metrics} --loss ${loss} --warmup ${warmup} --lrscheduler_start ${lrscheduler_start} --lrscheduler_step ${lrscheduler_step} --lrscheduler_decay ${lrscheduler_decay} \
    --dataset_mean ${dataset_mean} --dataset_std ${dataset_std} --audio_length ${audio_length} --noise ${noise}
  ...
  CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run.py --model ${model} --dataset ${dataset} \
    ...
    --tstride $tstride --fstride $fstride --imagenet_pretrain $imagenetpretrain --audioset_pretrain $audiosetpretrain \
    --metrics ${metrics} --loss ${loss} --warmup ${warmup} --lrscheduler_start ${lrscheduler_start} --lrscheduler_step ${lrscheduler_step} --lrscheduler_decay ${lrscheduler_decay} \
    --dataset_mean ${dataset_mean} --dataset_std ${dataset_std} --audio_length ${audio_length} --noise ${noise} --supcon True
