...
python ./prep_esc50.py --label_map_json $label_map --train_csv $train_csv --valid_csv $valid_csv --fold_wise_eval $fold_wise_eval --dataset_name $dataset
...
if [ "$fold_wise_eval" == "True" ]; then
  ...
  te_data=/fs/nexus-projects/brain_project/aaai_2025/ast/egs/esc50/data/datafiles_${dataset}/${dataset}_eval_data.json

  CUDA_CACHE_DISABLE=1 python -W ignore ../../src/run.py --model ${model} --dataset ${dataset} \
  --data-train ${tr_data} --data-val ${te_data} --exp-dir $exp_dir \
  --label-csv ./data/${dataset}_class_labels_indices.csv --n_class 50 \
  --lr $lr --n-epochs ${epoch} --batch-size $batch_size --save_model False \
  --freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} \
  --tstride $tstride --fstride $fstride --imagenet_pretrain $imagenetpretrain --audioset_pretrain $audiosetpretrain \
  --metrics ${metrics} --loss ${loss} --warmup ${warmup} --lrscheduler_start ${lrscheduler_start} --lrscheduler_step ${lrscheduler_step} --lrscheduler_decay ${lrscheduler_decay} \
