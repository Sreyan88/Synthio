...
def train(audio_model, train_loader, test_loader, args):
    ...
    # optimizer = torch.optim.AdamW(trainables, args.lr, weight_decay=0.1, betas=(0.95, 0.999)) # original for stablerep
    ...
    epoch += 1
    # for amp
    # scaler = NativeScalerWithGradNormCount() # original for stablerep
    scaler = GradScaler()
    ...
    while epoch < args.n_epochs + 1:
        ...
        # optimizer.zero_grad() # for stablerep
        for i, (audio_input, gt_labels) in enumerate(train_loader):

            B = audio_input.size(0)
            audio_input = audio_input.to(device, non_blocking=True)
            labels = torch.arange(B).to(device, non_blocking=True)
            labels = labels.view(-1, 1)
            labels = labels.repeat(4, 1).squeeze() # hard code
            gt_labels = gt_labels.view(-1, 1).squeeze()
            gt_labels = gt_labels.repeat(4, 1).view(1,-1).squeeze()

            # print(gt_labels)

            # first several steps for warm-up # global_step <= 1000 for original
            ...
            with autocast():
                ...
                feats = F.normalize(audio_output, dim=-1, p=2)
                local_batch_size = feats.size(0)

                # No need for all_gather as we're on a single GPU
                all_feats = feats
                all_labels = labels

                # Compute the mask based on labels
                mask = torch.eq(labels.view(-1, 1),
                                all_labels.view(1, -1)).float().to(device)
                ...
                for m in range(mask.shape[0]):
                    label_of_item = gt_labels[m]
                    for k in range(mask.shape[1]):
                        label_of_second_item = gt_labels[k]
                        if label_of_item == label_of_second_item:
                            if m!=k:
                                mask_clone[m][k] = 1.0


                mask = mask_clone # keep mask clone

                # Compute logits
                logits = torch.matmul(feats, all_feats.T) / 0.1
            ...
            # scaler(loss, optimizer, parameters=audio_model.parameters())
            # optimizer.zero_grad()
            # for name, param in audio_model.named_parameters():
            #     if param.grad is not None:
            #         print(f'Layer: {name} | Grad Norm: {param.grad.norm()}')
            # # original for stablerep above
            #--------------------#
            optimizer.zero_grad()
        ...
        torch.save(audio_model.state_dict(), "%s/models/supcon_audio_model.pth" % (exp_dir))
        if len(train_loader.dataset) > 2e5:
